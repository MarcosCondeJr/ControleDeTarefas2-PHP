...
<body>
    <div class="container">
        ...
        <form action="<?= BASE_URL ?>/update-tarefa" method="POST">
            <h1 class="mt-3 mb-3">Editar Tarefa</h1>
            <div class="row">
                <div class="col-sm-1 mb-3">
                    <label for="cd_tarefa" class="form-label">Código</label>
                    <input style="background-color:rgb(232, 231, 231)" type="text" class="form-control" name="cd_tarefa" value="<?= $_POST['cd_tarefa'] ?? $tarefa['cd_tarefa'] ?>" readonly>
                </div>
                <div class="col-sm-5 mb-3">
                    <label for="titulo_tarefa" class="form-label required">Titulo</label>
                    <input type="text" class="form-control" name="titulo_tarefa" value="<?= $_POST['titulo_tarefa'] ?? $tarefa['titulo_tarefa'] ?>">
                </div>
                <div class="col">
                        ...
                        <option></option>
                                <?php foreach ($usuarios as $usuario): ?>
                                    <option value="<?= $usuario['id_usuario'] ?? $_POST['id_usuario']?>"
                                        <?php 
                                            if (isset($tarefa['id_usuario']) && $usuario['id_usuario'] == $tarefa['id_usuario']) {
                                                echo 'selected';
                                            }
                                        ?>>
                                        <?= $usuario['nm_usuario'] ?>
                        ...
                        <option></option>
                                <?php foreach ($categorias as $cat): ?>
                                    <option value="<?= $cat['id_categoria'] ?? $_POST['id_categoria'] ?>"
                                        <?php 
                                            if (isset($tarefa['id_categoria']) && $cat['id_categoria'] == $tarefa['id_categoria']) {
                                                echo 'selected';
                                            }
                                        ?>>
                                        <?= $cat['nm_categoria'] ?>
            ...
            <div class="row">
                <div class="col mb-4">
                    ...
                    <textarea id="" class="form-control" name="ds_tarefa"><?= $_POST['ds_tarefa'] ?? $tarefa['ds_tarefa'] ?></textarea>
                </div>
            </div>
